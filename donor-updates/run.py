...
# ---------------------------------------------------------------------------
REDCROSS_DSN = dict(
    host=os.getenv("REDCROSS_HOST", "localhost"),
    port=int(os.getenv("REDCROSS_PORT", "5434")),
    dbname=os.getenv("REDCROSS_DB", "redcross_db"),
    user=os.getenv("REDCROSS_USER", "redcross_user"),
    password=os.getenv("REDCROSS_PASSWORD", "redcross_pass"),
)
WHO_DSN = dict(
    host=os.getenv("WHO_HOST", "localhost"),
    port=int(os.getenv("WHO_PORT", "5435")),
    dbname=os.getenv("WHO_DB", "who_db"),
    user=os.getenv("WHO_USER", "who_user"),
    password=os.getenv("WHO_PASSWORD", "who_pass"),
)

DONORS_PER_MINUTE = int(os.getenv("DONORS_PER_MINUTE", "10"))
SLEEP_SECONDS = int(os.getenv("SLEEP_SECONDS", "60"))
BATCH_SIZE = int(os.getenv("BATCH_SIZE", "10"))

# ---------------------------------------------------------------------------
# Data pools (trimmed but realistic)
# ---------------------------------------------------------------------------
BLOOD_GROUP_DIST = {
    "O+": 37, "B+": 25, "A+": 22, "AB+": 7,
    "O-": 3, "B-": 3, "A-": 2, "AB-": 1,
...
def rnd_date_after(start_dt: datetime) -> date:
    return rnd_dt_between(start_dt.date(), date.today()).date()


def rnd_address(city: str, state: str, pincode: str) -> str:
    num = random.randint(1, 999)
    street = random.choice(["Main Road", "Ring Road", "MG Road", "Market Road"])
    locality = random.choice(["Nagar", "Colony", "Layout", "Extension"])
    return f"{num}, {city} {locality}, {street}, {city}, {state} - {pincode}"
...
def insert_redcross_donors(conn, bank_ids, n):
    batch = []
    for _ in range(n):
        city, state, _w, pincodes = pick_city()
        pincode = random.choice(pincodes)
        age = random.randint(18, 65)
        created_at = rnd_donor_created_at(age)
        updated_at = rnd_updated_at(created_at)
        last_don = rnd_date_after(created_at)
        batch.append((
            random.choice(bank_ids),
            full_name(),
            unique_id(),
            rnd_mobile(),
            rnd_address(city, state, pincode),
            pick_blood_group(),
            age,
            last_don,
            created_at,
...
def insert_who_donors(conn, bank_ids, n):
    batch = []
    for _ in range(n):
        city, state, _w, pincodes = pick_city()
        pincode = random.choice(pincodes)
...
def main():
    print("Donor updates service started")
    rc_conn = psycopg2.connect(**REDCROSS_DSN)
    who_conn = psycopg2.connect(**WHO_DSN)

    try:
        rc_bank_ids = fetch_bank_ids(rc_conn)
        who_bank_ids = fetch_bank_ids(who_conn)
        if not rc_bank_ids or not who_bank_ids:
            raise RuntimeError("No banks found. Seed banks before running updates.")

        while True:
            insert_redcross_donors(rc_conn, rc_bank_ids, DONORS_PER_MINUTE)
            insert_who_donors(who_conn, who_bank_ids, DONORS_PER_MINUTE)
            print(f"Inserted {DONORS_PER_MINUTE} donors into Redcross and WHO")
