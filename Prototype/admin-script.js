...
function validateCredentials(adminId, password, region) {
  ...
  // Map dropdown values to full region names
  var regionMap = {
    'north': 'North Region',
    'south': 'South Region', 
    'east': 'East Region',
  ...
  var mappedRegion = regionMap[region] || region;
  
  // Check if region matches (or if admin has access to all regions)
  if (user.region !== 'All Regions' && user.region !== mappedRegion) {
    return false;
  }
...
function setupUserSession(adminId, selectedRegion) {
  ...
  // Map dropdown values to full region names for display
  var regionMap = {
    'north': 'North Region',
    'south': 'South Region', 
    'east': 'East Region',
  ...
  var displayRegion = regionMap[selectedRegion] || selectedRegion;
  ...
  updateHeaderInfo(userData, displayRegion);
