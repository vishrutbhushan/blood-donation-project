...
BANKS_PER_RUN = 500
DONORS_PER_RUN = 1000000
INVENTORY_TXN_PER_RUN = 1000000
BATCH_SIZE = 50000
RANDOM_SEED = 42
...
def load_pincode_pool():
    ...
    body = re.sub(
        r"\s*;\s*module\.exports\s*=\s*pincodelist\s*;\s*$", "", body, flags=re.DOTALL
    )
...
if PINCODE_POOL:
    _pincode_list = sorted(PINCODE_POOL)
    _filtered = []
    _removed_total = 0
    ...
    print(
        f"[Pincode] Filtered city pincodes against pincode.js — removed {_removed_total} invalid codes"
    )
...
def rnd_email(institution_name):
    ...
    domain = random.choice(
        ["gmail.com", "yahoo.co.in", "rediffmail.com", "hotmail.com", "outlook.com"]
    )
...
def unique_id():
    unique_id.counter += 1
    payload = f"seed:{RANDOM_SEED}:{unique_id.counter}:{random.random():.12f}".encode(
        "utf-8"
    )
...
def inventory_qty(blood_group, component):
    rarity_scale = BLOOD_GROUP_DIST[blood_group]
    base_max = max(2, int(rarity_scale * 1.5))
...
def make_source_event_id(
    source, bb_id, blood_group, component, sequence, event_timestamp, txn_type
):
...
def generate_inventory_transactions_batch(source, all_bb_ids, components, target_count):
    ...
    while generated < target_count:
        ...
        if txn_type == "INFLOW":
            ...
            if running_balance <= 0:
                ...
                delta = -random.randint(
                    1, min(running_balance, max(1, running_balance // 2))
                )
        else:
            delta = -random.randint(
                1, min(running_balance, max(1, running_balance // 2))
        ...
        source_event_id = make_source_event_id(
            source, bb_id, bg, component, sequence, event_time, txn_type
        )
...
def seed_redcross(conn):
    ...
    print(
        f"[Red Cross] Generating {INVENTORY_TXN_PER_RUN:,} inventory transactions (updated_at: {INVENTORY_DATE_START} → {INVENTORY_DATE_END}) ..."
    )
    inv_inserted = 0
    for batch in generate_inventory_transactions_batch(
        "redcross", all_bb_ids_for_inv, REDCROSS_COMPONENTS, INVENTORY_TXN_PER_RUN
    ):
        execute_values(
            cur,
        ...
        print(
            f"  [Red Cross] inventory txns: {inv_inserted:,} / {INVENTORY_TXN_PER_RUN:,}",
            end="\r",
            flush=True,
        )
    ...
    print(
        f"[Red Cross] Total banks in DB: {len(all_bb_ids)} — inserting {DONORS_PER_RUN:,} donors ..."
    )
    ...
    while inserted < DONORS_PER_RUN:
        ...
        inserted += batch_n
        print(
            f"  [Red Cross] donors: {inserted:,} / {DONORS_PER_RUN:,}",
            end="\r",
            flush=True,
        )
...
def seed_who(conn):
    ...
    all_bb_ids_for_inv = [row[0] for row in cur.fetchall()]
    print(
        f"[WHO] Generating {INVENTORY_TXN_PER_RUN:,} inventory transactions (updated_at: {INVENTORY_DATE_START} → {INVENTORY_DATE_END}) ..."
    )
    inv_inserted = 0
    for batch in generate_inventory_transactions_batch(
        "who", all_bb_ids_for_inv, WHO_COMPONENTS, INVENTORY_TXN_PER_RUN
    ):
        execute_values(
            cur,
            """INSERT INTO inventory_transaction
        ...
        inv_inserted += len(batch)
        print(
            f"  [WHO] inventory txns: {inv_inserted:,} / {INVENTORY_TXN_PER_RUN:,}",
            end="\r",
            flush=True,
        )
    ...
    all_bb_ids = [row[0] for row in cur.fetchall()]
    print(
        f"[WHO] Total banks in DB: {len(all_bb_ids)} — inserting {DONORS_PER_RUN:,} donors ..."
    )
    ...
    while inserted < DONORS_PER_RUN:
        ...
        inserted += batch_n
        print(
            f"  [WHO] donors: {inserted:,} / {DONORS_PER_RUN:,}", end="\r", flush=True
        )
...
def main():
    print(
        f"Realistic DB seeder  (BANKS_PER_RUN={BANKS_PER_RUN}, DONORS_PER_RUN={DONORS_PER_RUN:,}, INVENTORY_TXN_PER_RUN={INVENTORY_TXN_PER_RUN:,})\n"
    )
